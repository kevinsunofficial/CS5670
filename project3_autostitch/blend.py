...
def imageBoundingBox(img, M):
    ...
    tl, tr = M@np.array([0,0,1]), M@np.array([0,h-1,1])
    bl, br = M@np.array([w-1,0,1]), M@np.array([w-1,h-1,1])
    tl, tr, bl, br = tl/tl[-1], tr/tr[-1], bl/bl[-1], br/br[-1]

    minX, maxX = min(tl[0],tr[0],bl[0],br[0]), max(tl[0],tr[0],bl[0],br[0])
    minY, maxY = min(tl[1],tr[1],bl[1],br[1]), max(tl[1],tr[1],bl[1],br[1])
...
def accumulateBlend(img, acc, M, blendWidth):
    ...
    ih, iw, channel = img.shape
    useimg = np.zeros((ih,iw,channel+1))
    accimg = np.zeros(acc.shape)
    ah, aw, ac = acc.shape
    minx, miny, maxx, maxy = imageBoundingBox(img,M)
    
    for c in range(channel):
        p = img[:,:,c]
        useimg[:,:,c] = (p-np.min(p))/(np.max(p)-np.min(p))*255
    useimg[:,:,-1] = 1

    for c in range(channel+1):
        accimg[:,:,c] = cv2.warpPerspective(useimg[:,:,c],M,dsize=(aw,ah))

    for i in range(minx,maxx):
        if i < minx+blendWidth: accimg[:,i] *= (i-minx)/blendWidth
        elif i>maxx-blendWidth: accimg[:,i] *= (maxx-1-i)/blendWidth
    
    temp = np.sum(accimg[:,:,:-1],axis=2)
    accimg[:,:,-1] = np.where(temp!=0,accimg[:,:,-1],accimg[:,:,-1]*0)
...
def normalizeBlend(acc):
    ...
    h, w, channel = acc.shape
    img = np.zeros(acc.shape,dtype=np.uint8) # https://github.com/opencv/opencv/issues/6646
    for y in range(h):
        for x in range(w):
            if acc[y,x,-1]!=0: img[y,x] = acc[y,x]/acc[y,x,-1]
            img[y,x,-1] = 1
...
def getAccSize(ipv):
    ...
    for i in ipv:
        ...
        iminx, iminy, imaxx, imaxy = imageBoundingBox(img,M)
        minX, minY, maxX, maxY = min(iminx,minX), min(iminy,minY), max(imaxx,maxX), max(imaxy,maxY)
