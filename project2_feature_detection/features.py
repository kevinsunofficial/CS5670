...
class HarrisKeypointDetector(KeypointDetector):
    ...
    def computeHarrisValues(self, srcImage):
        ...
        dx, dy = ndimage.sobel(srcImage,1), ndimage.sobel(srcImage,0)
        orientationImage = np.degrees(np.arctan2(dy,dx))

        dx2, dy2, dxdy = dx**2, dy**2, dx*dy
        wdx2 = ndimage.gaussian_filter(dx2,sigma=0.5)
        wdy2 = ndimage.gaussian_filter(dy2,sigma=0.5)
        wdxdy = ndimage.gaussian_filter(dxdy,sigma=0.5)

        harrisImage = (wdx2*wdy2-wdxdy**2)-0.1*(wdx2+wdy2)**2
...
class SimpleFeatureDescriptor(FeatureDescriptor):
    ...
    def describeFeatures(self, image, keypoints):
        ...
        for i, f in enumerate(keypoints):
            ...
            use_img = np.pad(grayImage,pad_width=((2,2),(2,2)),mode='constant',constant_values=0)
            desc[i] = use_img[y:y+5,x:x+5].flatten()
...
class MOPSFeatureDescriptor(FeatureDescriptor):
    ...
    def describeFeatures(self, image, keypoints):
        ...
        for i, f in enumerate(keypoints):
            ...
            r = transformations.get_rot_mx(0,0,-rad)
            ...
            desc[i] = np.zeros(windowSize**2) if abs(np.var(destImage))<=1e-10 else (destImage/np.std(destImage)).flatten()
...
class SSDFeatureMatcher(FeatureMatcher):
    def matchFeatures(self, desc1, desc2):
        ...
        for i in range(desc1.shape[0]):
            j = np.argmin(dist[i])
            matches.append(cv2.DMatch(queryIdx=i,trainIdx=j,distance=dist[i,j]))
...
class RatioFeatureMatcher(FeatureMatcher):
    def matchFeatures(self, desc1, desc2):
        ...
        for i in range(desc1.shape[0]):
            ...
            j2 = np.argmin(dist[i])
            dist2 = dist[i,j2]
            matches.append(cv2.DMatch(_queryIdx=i,_trainIdx=j1,_distance=dist1/dist2))
